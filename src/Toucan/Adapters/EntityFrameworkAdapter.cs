...
namespace Toucan.Adapters
{
    public class EntityFrameworkAdapter<TStoreType, TKey> : DbAdapter<TStoreType, TKey> where TStoreType : DbContext
    {
        ...
        {
            IQueryable<IDbEntity<TKey>> dbSet = GetDBSetProperty(modelType);  
            var model = dbSet.FirstOrDefault(m => m.Id.Equals(key));            
            return model;
        }
        
        private IQueryable<IDbEntity<TKey>> GetDBSetProperty(Type modelType)
        {
            ...
            return property.GetGetMethod().Invoke(DataAccess, new object[0]) as IQueryable<IDbEntity<TKey>>;
        }
    }
}